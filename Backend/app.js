...
const SensorModel = require('./sensor_final')
...
const JWT_SECRET =
  ...
  mongoose.connect("mongodb://127.0.0.1:27017/library_database3")
  .then(() => {
...
app.post('/addsensordata', (req, res) => {
  const { temperature, humidity } = req.query; // Extract parameters from query string
  const sensorData = { temperature, humidity }; // Construct user data object
  ...
  SensorModel.find()
    .then(sensorData => {
      const formattedSensorData = sensorData.map(data => ({
        ...data.toJSON(),
        createdAt_formatted: data.createdAt_formatted,
        updatedAt_formatted: data.updatedAt_formatted
      }));
      console.log(formattedSensorData); // Log fetched sensor data with formatted timestamps
      res.json(formattedSensorData);
    })
    .catch(err => res.json(err));
});

app.get('/sensordata_temp', (req, res) => {
  SensorModel.find({}, 'temperature timestamp') // Select only the 'temperature' field
    .then(sensorData => {
      console.log(sensorData); // Log fetched sensor data
      res.json(sensorData);
    })
    .catch(err => res.json(err));
});

app.get('/sensordata_hm', (req, res) => {
  SensorModel.find({}, 'humidity timestamp') // Select only the 'temperature' field
    .then(sensorData => {
      console.log(sensorData); // Log fetched sensor data
      res.json(sensorData);
    })
...
app.get('/getReportDataTemp', async (req, res) => {
  try {
    const temperatureData = await SensorModel.aggregate([
      { $match: { temperature } },
      { $group: { _id: "$timestamp", count: { $sum: 1 } } }
    ]);
